...
class TestDbChatMessageHistory:
    """测试 MySQL 聊天历史存储"""

    def setup_method(self):
        self.session_id = "test_session_001"
        self.history = get_history(self.session_id)
        self.history.clear()
    ...
    def test_empty_history(self):
        """新会话应返回空列表"""
        messages = get_history("test_empty").messages
    ...
    def test_messages_persist_across_instances(self):
        """消息应持久化到 MySQL，新建实例也能读取"""
        msg = HumanMessage(content="持久化测试")
        self.history.add_messages([msg])

        new_history = get_history(self.session_id)
