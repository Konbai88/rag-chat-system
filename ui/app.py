...
with st.sidebar:
    ...
    if st.button("🗑️ 清空当前知识库", type="primary"):
    ...
    if st.button("❌ 删除该文件"):
        if not filename_to_del:
            ...
            with st.spinner(f"正在删除 {filename_to_del} ..."):
                ...
                st.success(f"✅ 文件 {filename_to_del} 已删除")
                st.rerun()

    if st.button("🔧 重建 ES 索引"):
        with st.spinner("正在重建索引，请勿关闭..."):
